...
#
# Flanking sweep with TWO approaches:
#   masking  : run masked sequence through ESM2, extract hidden states at segment
#   mean_act : run full sequence, then mean-ablate non-flank hidden states
#
# Run: .plm_nn/bin/python sse_probe.py [--protein 2B61A]
# =============================================================================
...
_p.add_argument("--protein", default="2B61A")
...
SS8_TO_SS3 = {"H": 0, "G": 0, "I": 0,   # helix
               "E": 1, "B": 1,            # strand
               "T": 2, "S": 2, "C": 2, "L": 2, "-": 2}  # coil
SS3_NAMES  = ["H (helix)", "E (strand)", "C (coil)"]
...
print(f"Protein : {protein}  (len={len(sequence_S)})")
...
def mask_with_flanks(seq_S: str, seg: ContactSegment, flank: int) -> str:
    ...
    Keeps: ss1 segment, ss2 segment, LEFT outer flank (before ss1),
    RIGHT outer flank (after ss2).
    """
    n = len(seq_S)
...
# ── Original contact map (baseline) ──────────────────────────────────────────

print("\nComputing original contact map (full sequence)...")
orig_contacts_AA = compute_contact_map(esm_model, tokenizer, sequence_S, device)
print(f"  orig_contacts_AA: {orig_contacts_AA.shape}")

# =============================================================================
# 1. Data — lamm-mit/protein_secondary_structure_from_PDB
#    126k sequences, 8-class DSSP in `Secondary_structure`, single train split
# =============================================================================
...
def _to_rows(hf_ds) -> list[dict]:
    return [{"sequence": r["Sequence"], "secondary_structure": r["Secondary_structure"]}
            for r in hf_ds
            if r["Sequence"] and r["Secondary_structure"]]

random.seed(42)
_train_all = _to_rows(train_ds)
_val_all   = _to_rows(val_ds)
_test_all  = _to_rows(test_ds)

train_rows = random.sample(_train_all, min(N_TRAIN, len(_train_all)))
val_rows   = random.sample(_val_all,   min(N_VAL,   len(_val_all)))
test_rows  = random.sample(_test_all,  min(N_TEST,  len(_test_all)))

print(f"Sizes — train: {len(train_rows):,}, val: {len(val_rows):,}, test: {len(test_rows):,}")


def row_to_ss3(row: dict) -> tuple[str, list[int]]:
    """Returns (sequence, list-of-SS3-ints) with lengths aligned."""
    seq = row["sequence"]
...
def extract_residue_embeddings(
    ...
    """
    Returns:
        embs_RD  : (total_residues, hidden_dim) — one vector per residue
        labels_R : (total_residues,)             — SS3 label per residue
    """
    ...
    for i, row in enumerate(rows):
        ...
        actual_len = enc["input_ids"].shape[1] - 2   # exclude BOS/EOS

        with torch.no_grad():
            out = model.esm(**enc, output_hidden_states=True)
            h   = out.hidden_states[layer][0, 1:actual_len+1]  # [A, D]

        labs = labs_full[:actual_len]
    ...
    print()
    embs   = np.concatenate(all_embs,  axis=0)   # [R, D]
...
train_embs, train_labels = extract_residue_embeddings(
    train_rows, esm_model, tokenizer, device,
    cache_path=f"{PROBE_CACHE_DIR}/train.npz")
val_embs,   val_labels   = extract_residue_embeddings(
    val_rows, esm_model, tokenizer, device,
    cache_path=f"{PROBE_CACHE_DIR}/val.npz")
test_embs,  test_labels  = extract_residue_embeddings(
    test_rows, esm_model, tokenizer, device,
    cache_path=f"{PROBE_CACHE_DIR}/test.npz")

print(f"\nResidues — train: {len(train_labels):,}, val: {len(val_labels):,}, "
      f"test: {len(test_labels):,}")
print(f"Class distribution (train): "
      f"H={np.mean(train_labels==0):.2f}, "
      f"E={np.mean(train_labels==1):.2f}, "
      f"C={np.mean(train_labels==2):.2f}")
...
# =============================================================================
# 4. Mean activation (used by mean-act approach)
# =============================================================================

_mean_act_cache = f"{PROBE_CACHE_DIR}/mean_activation.pt"
_pfam_mean_act  = "reports/cache/probe/mean_activation.pt"   # reuse if available
...
if os.path.exists(_mean_act_cache):
    ...
    mean_act_D = torch.cat(all_h, dim=0).mean(0).to(device)  # [D]
    torch.save(mean_act_D, _mean_act_cache)
print(f"Mean activation shape: {mean_act_D.shape}")

# =============================================================================
# 5. Get full-sequence SSE ground truth (no ablation)
# =============================================================================

def get_full_sse_preds(sequence_S: str) -> np.ndarray:
    """Full-sequence SSE predictions — ground truth for probe."""
    enc = tokenizer(sequence_S, return_tensors="pt",
                    truncation=True, max_length=1022).to(device)
    actual_len = enc["input_ids"].shape[1] - 2
    with torch.no_grad():
        out = esm_model.esm(**enc, output_hidden_states=True)
        h   = out.hidden_states[-1][0, 1:actual_len+1]   # [A, D]
    return sse_probe.predict(h.cpu().float().numpy())     # [A]


print(f"\nComputing full-sequence SSE ground truth for {protein}...")
...
print(f"  SS1 ground truth: {''.join('HEC'[p] for p in gt_ss1)}")
print(f"  SS2 ground truth: {''.join('HEC'[p] for p in gt_ss2)}")

# =============================================================================
# 6. SSE probe on segment — MEAN-ACT approach
#    Run full sequence → ablate non-(segment+flank) hidden states → probe
#    Flank definition matches mask_with_flanks:
#      keep left outer flank (before ss1) + both segments + right outer flank (after ss2)
# =============================================================================

def sse_probe_segment_mean_act(
    sequence_S : str,
    seg        : ContactSegment,
    flank_size : int,
    ...
    """
    Mean-ablation approach: run full sequence through ESM2,
    replace non-kept hidden states with mean_act, run SSE probe at segment.
    """
    enc     = tokenizer(sequence_S, return_tensors="pt",
                        truncation=True, max_length=1022).to(device)
    seq_len = enc["input_ids"].shape[1]

    # Which token positions to keep (1-indexed for BOS offset)
    keep = torch.zeros(seq_len, dtype=torch.bool, device=device)

    # ss1 segment
    for pos in range(seg.ss1_start, seg.ss1_end):
        tok_pos = pos + 1
        if tok_pos < seq_len - 1:
            keep[tok_pos] = True
    # ss2 segment
    for pos in range(seg.ss2_start, seg.ss2_end):
        tok_pos = pos + 1
        if tok_pos < seq_len - 1:
            keep[tok_pos] = True
    # left outer flank (before ss1)
    for pos in range(max(0, seg.ss1_start - flank_size), seg.ss1_start):
        tok_pos = pos + 1
        if tok_pos < seq_len - 1:
            keep[tok_pos] = True
    # right outer flank (after ss2)
    for pos in range(seg.ss2_end, min(seq_len - 2, seg.ss2_end + flank_size)):
        tok_pos = pos + 1
        if tok_pos < seq_len - 1:
            keep[tok_pos] = True
    ...
    with torch.no_grad():
        ...
        ablated = h_LD.clone()
        ablated[~keep] = mean_act_D                 # ablate non-kept positions
    ...
    for seg_name, seg_start, seg_end in [
        ...
        seg_h = ablated[seg_start+1 : seg_end+1]   # +1 for BOS offset
        if seg_h.shape[0] == 0:
            continue
...
# =============================================================================
# 7. SSE probe on segment — MASKING approach
#    Run masked sequence through ESM2 → extract hidden states at segment → probe
# =============================================================================

def sse_probe_segment_masking(
    sequence_S : str,
    seg        : ContactSegment,
    flank_size : int,
) -> dict:
    """
    Masking approach: run the masked sequence (with <mask> tokens) through ESM2.
    Single forward pass; extracts ss1, ss2, flk_left, flk_right from h_LD.
    flk_left  = left outer flank  [ss1_start-flank : ss1_start]
    flk_right = right outer flank [ss2_end         : ss2_end+flank]
    """
    n          = len(sequence_S)
    ...
    flk_l_end   = seg.ss1_start
    flk_r_start = seg.ss2_end
    ...
    for seg_name, seg_start, seg_end in [
        ("ss1",       seg.ss1_start, seg.ss1_end),
        ("ss2",       seg.ss2_start, seg.ss2_end),
        ("flk_left",  flk_l_start,   flk_l_end),
        ("flk_right", flk_r_start,   flk_r_end),
    ]:
        seg_h = h_LD[seg_start+1 : seg_end+1]   # +1 for BOS offset
        if seg_h.shape[0] == 0:
            continue
        seg_h_np = seg_h.cpu().float().numpy()
        results[seg_name] = {
...
sse_results  = []
all_mk_preds = {}   # keyed by flank; used in focused residue-level analysis below
...
for flank in range(FLANK_START, FLANK_END + 1):
    ...
    # masking approach — single forward pass returns ss1/ss2/flk_left/flk_right
    ...
    # flank ground truth: positions change per flank size → slice full_sse_preds_A
    flk_l_start = max(0, segment.ss1_start - flank)
    flk_l_end   = segment.ss1_start
    flk_r_start = segment.ss2_end
    flk_r_end   = min(len(sequence_S), segment.ss2_end + flank)

    gt_flk_left  = full_sse_preds_A[flk_l_start:flk_l_end]
    gt_flk_right = full_sse_preds_A[flk_r_start:flk_r_end]
...
# =============================================================================
# 8b. Focused residue-level prediction changes near clean flank (±2)
#     For each consecutive flank pair in [CLEAN_FLANK-2, CLEAN_FLANK+2],
#     report residues whose prediction flipped:
#       incorr→corr : was wrong at flank-1, now correct at flank
#       corr→incorr : was correct at flank-1, now wrong at flank
#     For flk_left/flk_right (growing segments) only compare overlapping
#     positions — the newly exposed residue is excluded from the diff.
# =============================================================================
...
for _flank in range(_FOCUS_LO, _FOCUS_HI + 1):
    ...
    # ── variable-size flanks (compare only overlapping residues) ─────────
    for _key, _lbl in [("flk_left", "flk_L"), ("flk_right", "flk_R")]:
        ...
        if _key == "flk_left":
            ...
            # new residue added at index 0 (leftmost); skip it when present
            if len(_pc) > len(_pp):
                _pc    = _pc[1:]
                _abs_c += 1
        else:  # flk_right
            _abs_c = segment.ss2_end
            # new residue added at the end; skip it when present
            if len(_pc) > len(_pp):
                _pc = _pc[:-1]
...
# =============================================================================
# 9. Plot — contact + both SSE approaches
# =============================================================================
...
# Panel 2: masking SSE accuracy — segments AND flanks together
...
plt.tight_layout()
outpath = f"{PLOT_DIR}/{protein}_sse_sweep.png"
