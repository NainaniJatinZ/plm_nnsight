...
def path_patching_pass_c(
    model, clean_inputs_BL, clean_attn_LBHLL, corrupt_attn_LBHLL,
    source_layer, source_head, device,
):
    """
    IOI-style path patching Pass C:
    - Clean forward, freeze ALL heads to clean attention, EXCEPT source head
      which gets corrupt attention.
    - MLPs and LayerNorm recompute from modified residual stream.
    - Capture downstream Q, K, and attention patterns.

    Returns dict with q_{layer}, k_{layer}, attn_{layer} for downstream layers.
    """
    with model.trace() as tracer:
        with tracer.invoke(**{**clean_inputs_BL, "output_attentions": True}):
            # CRITICAL: Call tracer.cache() BEFORE any interventions
            # (tracer.cache skips modules that already have hooks registered)
            cache_modules = []
            for i in range(source_layer + 1, NUM_LAYERS):
                ...
                cache_modules.append(model.esm.encoder.layer[i].attention.self)
            downstream_cache = tracer.cache(modules=cache_modules)

            # Freeze every layer's attention head outputs
            for l in range(NUM_LAYERS):
                # Get current V (recomputed from current residual stream)
                v_raw = model.esm.encoder.layer[l].attention.self.value.output
                v_heads = v_raw.reshape(B, L, NUM_HEADS, HEAD_DIM).transpose(1, 2)

                if l == source_layer:
                    # Source head: use corrupt attention, all others: clean
                    frozen_attn = clean_attn_LBHLL[l].to(device).clone()
                    frozen_attn[:, source_head] = corrupt_attn_LBHLL[l][:, source_head].to(device)
                else:
                    # All heads frozen to clean attention
                    frozen_attn = clean_attn_LBHLL[l].to(device)

                # Recompute context with frozen attention but CURRENT V
                # (V depends on residual stream, which MLPs have modified)
...
# Run path patching for each source head in the IE circuit
ie_circuit_heads = layer_head_indices  # your 45 (layer, head) tuples

edge_effects = {}  # (sl, sh, dl, dh) -> {q_change, k_change, q_by_pos, k_by_pos}

for src_idx, (sl, sh) in enumerate(ie_circuit_heads):
    results = path_patching_pass_c(
        model, clean_inputs_BL, clean_attn_LBHLL, corrupt_attn_LBHLL,
        sl, sh, device
    )
    ...
    for dl, dh in ie_circuit_heads:
        ...
        # Q change per position
        patched_q = results[f'q_{dl}'].reshape(B, L, NUM_HEADS, HEAD_DIM)[:, :, dh]
        clean_q = clean_qk[f'q_{dl}'].reshape(B, L, NUM_HEADS, HEAD_DIM)[:, :, dh]
        q_diff = (patched_q - clean_q).squeeze(0)  # (L, HEAD_DIM)
        q_by_pos = q_diff.norm(dim=-1)  # (L,) — how much Q changed at each position
        q_total = q_by_pos.sum().item()
        
        # K change per position
        patched_k = results[f'k_{dl}'].reshape(B, L, NUM_HEADS, HEAD_DIM)[:, :, dh]
        clean_k = clean_qk[f'k_{dl}'].reshape(B, L, NUM_HEADS, HEAD_DIM)[:, :, dh]
        k_diff = (patched_k - clean_k).squeeze(0)
        k_by_pos = k_diff.norm(dim=-1)
        k_total = k_by_pos.sum().item()
        
        if q_total > 0.01 or k_total > 0.01:  # threshold
            edge_effects[(sl, sh, dl, dh)] = {
                'q_total': q_total,
                'k_total': k_total,
                'q_by_pos': q_by_pos,  # (L,) — token-level!
                'k_by_pos': k_by_pos,  # (L,) — token-level!
            }
...
# %%
# Analyze edge_effects results
print(f"\n{'='*60}")
print(f"PATH PATCHING RESULTS SUMMARY")
print(f"{'='*60}")
print(f"\nTotal edges found: {len(edge_effects)}")
print(f"(edges = source head → dest head connections with Q or K change > 0.01)")

# Sort edges by total effect (q_total + k_total)
edges_with_total = [(edge, vals['q_total'] + vals['k_total']) for edge, vals in edge_effects.items()]
edges_with_total.sort(key=lambda x: x[1], reverse=True)

print(f"\nTop 20 edges by total Q+K change:")
print(f"{'Source':>15} → {'Dest':>15}  {'Q change':>10}  {'K change':>10}  {'Total':>10}")
print("-" * 70)
for (sl, sh, dl, dh), total in edges_with_total[:20]:
    q_tot = edge_effects[(sl, sh, dl, dh)]['q_total']
    k_tot = edge_effects[(sl, sh, dl, dh)]['k_total']
    print(f"L{sl:2d}H{sh:2d} → L{dl:2d}H{dh:2d}  {q_tot:>10.4f}  {k_tot:>10.4f}  {total:>10.4f}")

# Top edges by Q change only
edges_by_q = sorted(edge_effects.items(), key=lambda x: x[1]['q_total'], reverse=True)
print(f"\nTop 10 edges by Q change:")
for (sl, sh, dl, dh), vals in edges_by_q[:10]:
    print(f"  L{sl:2d}H{sh:2d} → L{dl:2d}H{dh:2d}: Q = {vals['q_total']:.4f}, K = {vals['k_total']:.4f}")

# Top edges by K change only
edges_by_k = sorted(edge_effects.items(), key=lambda x: x[1]['k_total'], reverse=True)
print(f"\nTop 10 edges by K change:")
for (sl, sh, dl, dh), vals in edges_by_k[:10]:
    print(f"  L{sl:2d}H{sh:2d} → L{dl:2d}H{dh:2d}: Q = {vals['q_total']:.4f}, K = {vals['k_total']:.4f}")

# Statistics
q_totals = [v['q_total'] for v in edge_effects.values()]
k_totals = [v['k_total'] for v in edge_effects.values()]
print(f"\nStatistics:")
print(f"  Q changes: mean={torch.tensor(q_totals).mean():.4f}, median={torch.tensor(q_totals).median():.4f}, max={max(q_totals):.4f}")
print(f"  K changes: mean={torch.tensor(k_totals).mean():.4f}, median={torch.tensor(k_totals).median():.4f}, max={max(k_totals):.4f}")

# Layer distribution
source_layers = [edge[0] for edge in edge_effects.keys()]
dest_layers = [edge[2] for edge in edge_effects.keys()]
print(f"\nLayer distribution:")
print(f"  Source layers: {min(source_layers)} to {max(source_layers)}")
print(f"  Dest layers: {min(dest_layers)} to {max(dest_layers)}")
